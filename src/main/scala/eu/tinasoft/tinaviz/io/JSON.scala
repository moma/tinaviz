 ...
 */
 ...
 trait JsonValue
 {
     ...
     * Override for JsonObject and JsonArray     
     */  
    ...
    /**
     * Every JsonValue type will be able to use the following methods, so that
     * we may chain apply()'s together as a path, like:
     * {
     *  "hello" :
     *      {
     *      "world" : [9,8,7,6,5,4]     
     ...
     * Provide no-val returns for inappropriate types
     */              
      ...
      this match
    {
      ...
      case JsonString(v)  => v.equalsIgnoreCase("true")
      case _              => false
    }
  }
 ...
 class JsonParser(debug: Boolean = false)
 {
    ...
    def trace(str: String) =
      if (debug) println("JsonParser: " + str)

    /**
     * Overrideable to allow message redirection, such as log.error()
     */         
    def error(pos: Int, str: String) =
      println("JsonParser error (" + pos + "): " + str)

    private val EOF = (-1).asInstanceOf[Char]

    private var parsebuf = ""
    
    private val doubleMatcher =
      new util.matching.Regex("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?") 

    private val hexMatcher = new util.matching.Regex("[0-9a-fA-F]{4}") 
    ...
    private def matchReg(pos: Int, regex:util.matching.Regex) = 
    ...
    private def quotedString(pos: Int) : (String, Int) =
    {
      ...
      while (p < parsebuf.size)
      {
        ...
        p += 1
        ch match
        { //escapes.  see http://json.org fo string escape sequences
          case '\\' =>
            {
              val ch2 = get(p)
              p += 1
    ...
    private def jsonEntry(pos: Int) : (String, JsonValue, Int) =
    {
      ...
      {
        val (name, qpos) = quotedString(p+1)
        if (qpos < 0)
          return ("", JsonNil, -1)
        else
        {
    ...
    {
      trace("jsonArray")
      val xs = collection.mutable.ListBuffer[JsonValue]()
      var p = skipwhite(pos)
      var first = true
      while (p < parsebuf.size)
      {
    ...
    private def jsonString(pos: Int) : (JsonValue, Int) =
    {
      trace("jsonString")
      val (str, spos) = quotedString(pos)
      if (spos < 0)
        (JsonNil, -1)
      else
        (JsonString(str), spos)
 ...
 object JsonParser
 {
    ...
    def parseURL(urls: String = "", debug: Boolean = false) : Option[JsonValue] =
    {
      ...
      {
        val buf = new StringBuilder
        io.Source.fromURL(new java.net.URL(urls)).foreach(buf.append)
